...
class Admin_model extends CI_Model
{
    ...
    public function ambil_data_by_id_with_pagination($perpage,$offset,$keyword)
    {
        $query = $this->db->select('*')
                          ->from('tb_blog')
                          ->join('tb_auth', 'tb_blog.user = tb_auth.id')
                          ->like('tb_blog.judul', $keyword)
                          ->order_by('tb_blog.id_blog', 'DESC')
